...
const manifest = JSON.parse(await readFile(join(root, "packages", "writeguard", "package.json"), "utf8"));
if (manifest.dependencies?.openai || manifest.optionalDependencies?.openai || manifest.peerDependencies?.openai) {
  throw new Error("@closure/writeguard manifest must not depend on the OpenAI SDK");
}
...
const output = await new Promise((resolveOutput, reject) => {
  const args = ["--filter", "@closure/writeguard", "list", "--prod", "--depth", "Infinity", "--json"];
  ...
  child.on("exit", (code) => code === 0 ? resolveOutput(stdout) : reject(new Error(stderr || `pnpm list exited ${code}`)));
});

const graph = JSON.parse(output);
const serialized = JSON.stringify(graph);
if (/"openai"\s*:/.test(serialized) || /node_modules[\\/]openai(?:[\\/]")?/.test(serialized)) {
  throw new Error("OpenAI SDK appeared in @closure/writeguard's production dependency graph");
}
const result = {
...
await writeFile(
  join(root, ".writeguard", "core-openai-boundary.json"),
  `${JSON.stringify(result, null, 2)}\n`,
  "utf8"
);
console.log(`Core OpenAI dependency boundary passed: ${JSON.stringify(result)}`);
